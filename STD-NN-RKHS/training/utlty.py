import numpy as np
import pickle as pkl

# this is a DataContainer class that stores data


class DataContainer:
    def __repr__(self):
        return "DataContainer"

    def __str__(self):
        return self._data.to_string()

    def __init__(self, nt, nv, seed=None, load_test=False):

        self._num_data = 2360  # Total number of data
# ---------------------------------------------------------------
# data structure of the input file
# input features for reactants (1 to 12)
# E(v,j), E(v,j=0), v, E(v=0,j), j, sqrt((j*(j+1))), E_tra, v_rel, q+, q-, centrifugal barrier height, time period
# input features for products (13 to 24)
# E(v',j'), E(v',j'=0), v, E(v'=0,j'), j', sqrt((j'*(j'+1))), E_tra, v_rel, q+, q-, centrifugal barrier height, time period
# output features
# cross section
# ---------------------------------------------------------------
        self._num_features = 11  # Total number of features 10
        self._num_outputs = 161  # Total number of output 15
        self._load_test = load_test  # whether to load the test set or not
        self._random_state = np.random.RandomState(seed=seed)

        # load the data set
        input_data = np.genfromtxt('input_for_neural_new.txt', delimiter=',')

        data = np.zeros((self._num_data, self._num_features + self._num_outputs))
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                data[i, j] = input_data[i, j]

#        data = np.genfromtxt('input_for_neural_withavg.txt', delimiter=',')
#        filename = 'input_for_neural.txt'
#        with open(filename) as f:
#           contents = f.read().splitlines()

#        data=np.zeros((len(contents),141), dtype=float)
#       store = [0,0,0,0,0,0]
#        print('IAMHERE')
#        for i in range(len(contents)):
#             atmp=contents[i].split(',')
#             data[i,0:141]=atmp[2:143]
#             print(atmp)
#             if len(atmp) < 28:
#               data[i,0:16]=atmp[0:16]
#               data[i,16:22]=atmp[20:26]
#               #data[i,30]=-4.0
#               #print(data[i,:])
#             else:
#               data[i,0:16]=atmp[0:16]
#               data[i,16:22]=atmp[24:30]
#               data[i,30]=4.0
#               print(data[i,:])

#        quit()
#            store=contents[i][20:25]
#            contents[i][20:23]=0.0
#            contents[i].append(store)
#        data = np.asarray(contents)
#        print(type(contents), type(data))
#        print(data.shape)

        # create shuffled list of indices
        indices = self._random_state.permutation(np.arange(0, self._num_data))

        # store indices of training, validation and test data
        indices_train = indices[0:nt]
        indices_valid = indices[nt:nt+nv]
        indices_test = indices[nt+nv:]

        np.savetxt('valid_indices.txt', indices_valid, delimiter=',')
        np.savetxt('test_indices.txt', indices_test, delimiter=',')

        # save number of training, validation and test data
        self._num_train = indices_train.shape[0]
        self._num_valid = indices_valid.shape[0]
        self._num_test = indices_test.shape[0]

        # store 3 different sets of data
        self._data_train = data[indices_train, :]
        self._data_valid = data[indices_valid, :]
        if self._load_test:
            self._data_test = data[indices_test, :]

        # calculate running mean/stdev to normalize the data
        n = 0
        S = np.zeros(self._num_features, dtype=float)
        m = np.zeros(self._num_features, dtype=float)
        for i in range(self._num_train):
            # loop through the descriptor and update mean/stdev
            n += 1  # keeps track of how many samples have been analyzed for mean/stdev
            for j in range(self._num_features):
                # update mean/stdev
                m_prev = m[j]
                m[j] += (self._data_train[i, j]-m[j])/n
                S[j] += (self._data_train[i, j]-m[j]) * (self._data_train[i, j]-m_prev)

        stdev = np.sqrt(S/n)

        np.savetxt('Coeff_mval_input.txt', m, delimiter=',')
        np.savetxt('Coeff_stdv_input.txt', stdev, delimiter=',')
#        m = np.genfromtxt('Coeff_mval.txt', delimiter=',')
#        stdev = np.genfromtxt('Coeff_stdv.txt', delimiter=',')

        # rescale features by standardization
        # training set
        with open("features_train.txt", "w") as file:
            for i in range(self._num_train):
                file.write(str(i) + '\t')
                for j in range(self._num_features):
                    if stdev[j] > 0.0:
                        self._data_train[i, j] = (self._data_train[i, j]-m[j])/stdev[j]
                        file.write(str(self._data_train[i, j]) + '\t')
                    else:
                        self._data_train[i, j] = (self._data_train[i, j]-m[j])
                        file.write(str(self._data_train[i, j]) + '\t')
                file.write('\n')

        # validation set
        with open("features_valid.txt", "w") as file:
            for i in range(self._num_valid):
                file.write(str(i) + '\t')
                for j in range(self._num_features):
                    if stdev[j] > 0.0:
                        self._data_valid[i, j] = (self._data_valid[i, j]-m[j])/stdev[j]
                        file.write(str(self._data_valid[i, j]) + '\t')
                    else:
                        self._data_valid[i, j] = (self._data_valid[i, j]-m[j])
                        file.write(str(self._data_valid[i, j]) + '\t')
                file.write('\n')

        # test set
        if self._load_test:
            for i in range(self._num_test):
                for j in range(self._num_features):
                    if stdev[j] > 0.0:
                        self._data_test[i, j] = (self._data_test[i, j]-m[j])/stdev[j]
                    else:
                        self._data_test[i, j] = (self._data_test[i, j]-m[j])


####################
        #
        # #calculate running mean/stdev to normalize the output
        #
        # coeff_Eout = []
        # coeff_vout = []
        # coeff_jout = []
        #
        # for i in range(self._num_train):
        #     for j in range(self._num_features, self._num_features + num_coeff_Eout):
        #         coeff_Eout.append(self._data_train[i,j])
        #     for j in range(self._num_features + num_coeff_Eout, self._num_features + num_coeff_Eout + num_coeff_vout):
        #         coeff_vout.append(self._data_train[i,j])
        #     for j in range(self._num_features + num_coeff_Eout + num_coeff_vout, self._num_features + self._num_outputs):
        #         coeff_jout.append(self._data_train[i,j])
        #
        # m_Eout = np.mean(coeff_Eout, axis=0, dtype=np.float64)
        # m_vout = np.mean(coeff_vout, axis=0, dtype=np.float64)
        # m_jout = np.mean(coeff_jout, axis=0, dtype=np.float64)
        # S_Eout = np.std(coeff_Eout, axis=0, dtype=np.float64)
        # S_vout = np.std(coeff_vout, axis=0, dtype=np.float64)
        # S_jout = np.std(coeff_jout, axis=0, dtype=np.float64)
        #
        # m = [m_Eout,m_vout,m_jout]
        # stdev = [S_Eout,S_vout,S_jout]
        #
        # self.mval = m
        # self.stdv = stdev
        #
        # np.savetxt('Coeff_mval_output.txt',m, delimiter=',')
        # np.savetxt('Coeff_stdv_output.txt',stdev, delimiter=',')
        #
        # #rescale features by standardization
        # #training set
        # with open("output_train.txt", "w") as file:
        #     for i in range(self._num_train):
        #         file.write(str(i) + '\t')
        #         for j in range(self._num_features, self._num_features + num_coeff_Eout):
        #             if S_Eout > 0.0:
        #                 self._data_train[i,j] = (self._data_train[i,j]-m_Eout)/S_Eout
        #                 file.write(str(self._data_train[i,j]) + '\t')
        #             else:
        #                 self._data_train[i,j] = (self._data_train[i,j]-m_Eout)
        #                 file.write(str(self._data_train[i,j]) + '\t')
        #
        #         for j in range(self._num_features + num_coeff_Eout, self._num_features + num_coeff_Eout + num_coeff_vout):
        #             if S_vout > 0.0:
        #                 self._data_train[i,j] = (self._data_train[i,j]-m_vout)/S_vout
        #                 file.write(str(self._data_train[i,j]) + '\t')
        #             else:
        #                 self._data_train[i,j] = (self._data_train[i,j]-m_vout)
        #                 file.write(str(self._data_train[i,j]) + '\t')
        #
        #         for j in range(self._num_features + num_coeff_Eout + num_coeff_vout, self._num_features + self._num_outputs):
        #             if S_jout > 0.0:
        #                 self._data_train[i,j] = (self._data_train[i,j]-m_jout)/S_jout
        #                 file.write(str(self._data_train[i,j]) + '\t')
        #             else:
        #                 self._data_train[i,j] = (self._data_train[i,j]-m_jout)
        #                 file.write(str(self._data_train[i,j]) + '\t')
        #
        #         file.write('\n')
        #
        # #validation set
        # with open("output_valid.txt", "w") as file:
        #     for i in range(self._num_valid):
        #         file.write(str(i) + '\t')
        #         for j in range(self._num_features, self._num_features + num_coeff_Eout):
        #             if S_Eout > 0.0:
        #                 self._data_valid[i,j] = (self._data_valid[i,j]-m_Eout)/S_Eout
        #                 file.write(str(self._data_valid[i,j]) + '\t')
        #             else:
        #                 self._data_valid[i,j] = (self._data_valid[i,j]-m_Eout)
        #                 file.write(str(self._data_valid[i,j]) + '\t')
        #
        #         for j in range(self._num_features + num_coeff_Eout, self._num_features + num_coeff_Eout + num_coeff_vout):
        #             if S_vout > 0.0:
        #                 self._data_valid[i,j] = (self._data_valid[i,j]-m_vout)/S_vout
        #                 file.write(str(self._data_valid[i,j]) + '\t')
        #             else:
        #                 self._data_valid[i,j] = (self._data_valid[i,j]-m_vout)
        #                 file.write(str(self._data_valid[i,j]) + '\t')
        #
        #         for j in range(self._num_features + num_coeff_Eout + num_coeff_vout, self._num_features + self._num_outputs):
        #             if S_jout > 0.0:
        #                 self._data_valid[i,j] = (self._data_valid[i,j]-m_jout)/S_jout
        #                 file.write(str(self._data_valid[i,j]) + '\t')
        #             else:
        #                 self._data_valid[i,j] = (self._data_valid[i,j]-m_jout)
        #                 file.write(str(self._data_valid[i,j]) + '\t')
        #
        #         file.write('\n')
        #
        # #test set
        # if self._load_test:
        #     for i in range(self._num_test):
        #         for j in range(self._num_features, self._num_features + num_coeff_Eout):
        #             if S_Eout > 0.0:
        #                 self._data_test[i,j] = (self._data_test[i,j]-m_Eout)/S_Eout
        #             else:
        #                 self._data_test[i,j] = (self._data_test[i,j]-m_Eout)
        #
        #         for j in range(self._num_features + num_coeff_Eout, self._num_features + num_coeff_Eout + num_coeff_vout):
        #             if S_vout > 0.0:
        #                 self._data_test[i,j] = (self._data_test[i,j]-m_vout)/S_vout
        #             else:
        #                 self._data_test[i,j] = (self._data_test[i,j]-m_vout)
        #
        #         for j in range(self._num_features + num_coeff_Eout + num_coeff_vout, self._num_features + self._num_outputs):
        #             if S_jout > 0.0:
        #                 self._data_test[i,j] = (self._data_test[i,j]-m_jout)/S_jout
        #             else:
        #                 self._data_test[i,j] = (self._data_test[i,j]-m_jout)

        n = 0
        S = np.zeros(self._num_outputs, dtype=float)
        m = np.zeros(self._num_outputs, dtype=float)
        for i in range(self._num_train):
            # loop through the descriptor and update mean/stdev
            n += 1  # keeps track of how many samples have been analyzed for mean/stdev
            for j in range(self._num_features, self._num_features + self._num_outputs):
                # update mean/stdev
                m_prev = m[j-self._num_features]
                m[j-self._num_features] += (self._data_train[i, j]-m[j-self._num_features])/n
                S[j-self._num_features] += (self._data_train[i, j] -
                                            m[j-self._num_features]) * (self._data_train[i, j]-m_prev)

        self.mval = m
        self.stdv = np.sqrt(S/n)
        stdev = np.sqrt(S/n)

        np.savetxt('Coeff_mval_output.txt', m, delimiter=',')
        np.savetxt('Coeff_stdv_output.txt', stdev, delimiter=',')
        #        m = np.genfromtxt('Coeff_mval.txt', delimiter=',')
        #        stdev = np.genfromtxt('Coeff_stdv.txt', delimiter=',')

        # rescale features by standardization
        # training set
        with open("output_train.txt", "w") as file:
            for i in range(self._num_train):
                file.write(str(i) + '\t')
                for j in range(self._num_features, self._num_features + self._num_outputs):
                    if stdev[j-self._num_features] > 0.0:
                        self._data_train[i, j] = (self._data_train[i, j]) / \
                            stdev[j-self._num_features]
                        file.write(str(self._data_train[i, j]) + '\t')
                    else:
                        self._data_train[i, j] = (self._data_train[i, j])
                        file.write(str(self._data_train[i, j]) + '\t')
                file.write('\n')

        # validation set
        with open("output_valid.txt", "w") as file:
            for i in range(self._num_valid):
                file.write(str(i) + '\t')
                for j in range(self._num_features, self._num_features + self._num_outputs):
                    if stdev[j-self._num_features] > 0.0:
                        self._data_valid[i, j] = (self._data_valid[i, j]) / \
                            stdev[j-self._num_features]
                        file.write(str(self._data_valid[i, j]) + '\t')
                    else:
                        self._data_valid[i, j] = (self._data_valid[i, j])
                        file.write(str(self._data_valid[i, j]) + '\t')
                file.write('\n')

        # test set
        if self._load_test:
            for i in range(self._num_test):
                for j in range(self._num_features, self._num_features + self._num_outputs):
                    if stdev[j-self._num_features] > 0.0:
                        self._data_test[i, j] = (self._data_test[i, j])/stdev[j-self._num_features]
                    else:
                        self._data_test[i, j] = (self._data_test[i, j])

#        ####################

        # for retrieving batches
        self._index_in_epoch = 0

    # total amount of data
    @property
    def num_data(self):
        return self._num_data

    # number of training examples
    @property
    def num_train(self):
        return self._num_train

    # number of validation examples
    @property
    def num_valid(self):
        return self._num_valid

    # number of test examples
    @property
    def num_test(self):
        return self._num_test

    @property
    def num_features(self):
        return self._num_features

    @property
    def num_outputs(self):
        return self._num_outputs

    # shuffles the training data
    def shuffle_train_data(self):
        indices = self._random_state.permutation(np.arange(0, self._num_train))
        self._data_train = self._data_train[indices, :]

    # returns a batch of samples from the training set
    def next_batch(self, batch_size=1):
        start = self._index_in_epoch
        self._index_in_epoch += batch_size
        # epoch is finished, test set needs to be shuffled
        if self._index_in_epoch > self.num_train:
            # shuffle training data
            self.shuffle_train_data()
            start = 0
            self._index_in_epoch = batch_size
            assert batch_size <= self._num_train
        end = self._index_in_epoch
        return self._data_train[start:end, :self._num_features], self._data_train[start:end, self._num_features:self._num_features+self._num_outputs]

    def get_train_data(self):
        return self._data_train[:, :self._num_features], self._data_train[:, self._num_features:self._num_features+self._num_outputs]

    def get_valid_data(self):
        return self._data_valid[:, :self._num_features], self._data_valid[:, self._num_features:self._num_features+self._num_outputs]

    def get_test_data(self):
        assert self._load_test
        return self._data_test[:, :self._num_features], self._data_test[:, self._num_features:self._num_features+self._num_outputs]

    def get_one_data(self, nindx):
        return self._data_valid[nindx, :self._num_features], self._data_valid[nindx, self._num_features:self._num_features+self._num_outputs]

    def get_onetest_data(self, nindx):
        return self._data_test[nindx, :self._num_features], self._data_test[nindx, self._num_features:self._num_features+self._num_outputs]

    def get_onetrain_data(self, nindx):
        return self._data_train[nindx, :self._num_features], self._data_train[nindx, self._num_features:self._num_features+self._num_outputs]

    def get_mval(self):
        return self.mval

    def get_stdv(self):
        return self.stdv
